...
const newCoupon = async (req, res) => {
  try {
    ...
    if (data) {
      ...
      res.render("addcoupon", {
        mesage: "percentage should be between 0 to 10!",
      });
  ...
  try {
    ...
    if (coupondata) {
      if (couponCode == "" || percentage.trim() == "") {
        ...
        res.render("editCoupon", {
          coupon: coupondata,
          message: "percentage should be between 0 to 10!!!",
        });
      } else if (expiry.getTime() <= Date.now()) {
        res.render("editCoupon", {
          coupon: coupondata,
          message: "Data should be valid",
        });
      } else {
        const update = await couponData.updateOne(
          { _id: id },
          {
            $set: {
              code: couponCode,
